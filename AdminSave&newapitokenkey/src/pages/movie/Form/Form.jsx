...
const Form = () => {
  ...
  const validateForm = () => {
    const errors = [];
    const requiredFields = ["title", "overview", "releaseDate", "popularity", "voteAverage"];
    
    requiredFields.forEach((field) => {
      if (!formData[field]) errors.push(`${field.charAt(0).toUpperCase() + field.slice(1)} is required`);
    });
    
    if (!selectedMovie) errors.push("select movie");
    return errors;
  };
  ...
  useEffect(() => {
    ...
    const fetchMovieDetails = async () => {
      try {
        ...
        const { data: movieData } = await axios.get(`/movies/${movieId}`);
